...
class Product extends Component {
  ...
  constructor(props, context) {
    ...
    this.state = {
      ...
      images: '',
      loadingVariation: '', 
   // Loading state fetch product variations
      quantity: 1,
      variation: Map(),
      meta_data: List(),
      variations: List(),
  ...
  getData = async() =>{
    ...
    const {dispatch, attribute, lang,navigation, data, currency, defaultCurrency} = this.props;
...
const products = navigation.getParam('product', {});
// console.log(products,'products')
const dataProduct = products && products.id ? fromJS(products) : dataPrepare;
this.setState({
  product: dataProduct,
  ...
  loadingVariation: dataProduct.get('type') === productType.VARIABLE, 
   ...
   const vendor_id = dataProduct.getIn(['store', 'id']);
    dispatch(fetchRating(dataProduct.get('id')));

    // Fetch attribute with product is variation
    if (
      dataProduct.get('type') === productType.VARIABLE &&
    ...
    // Fetch variations
    if (dataProduct.get('type') === productType.VARIABLE) {
      const CancelToken = axios.CancelToken;
      this.source = CancelToken.source();
      // Get variations
      getVariations(dataProduct.get('id'), lang, this.source.token)
        .then(data => {
          this.setState({
...
if(product){
  ...
  componentDidUpdate(prevProps) {
    ...
    if (this.props.navigation.getParam('product', {}).id !== prevProps.navigation.state.params.product.id) {
      // this.refs._scrollView.scrollTo(0);
      this.setState({loader:true})
      // this.refs._scrollView.scrollTo({ x:-400, y:-400 })
      this.getData();
      this.showInfoType()
      this.images()
    }
  ...
  render() {
    ...
    const related_ids = product?product.get('related_ids').size
    ...
    return (
      ...
      <ScrollProductDetail
        headerTitle={product?product.get('name'):''}
        imageElement={
          <ProductImages
            images={images}
            product_id={product?product.get('id'):''}
            url={product?product.get('permalink'):''}
            name_product={product?product.get('name'):''}
        ...
        <Container style={styles.container}>
          <View style={styles.viewCategoryRating}>
            ...
            {configs.get('toggleReviewProduct') ?
              <TouchableOpacity
                ...
                <Text style={styles.textRating}>({product?product.get('rating_count'):''})</Text>
          ...
          {this.showPrice()}
          {product && product.get('sku') ? <Text h6 medium style={styles.textName}>
            sku: {product?product.get('sku'):''}
          </Text> : null}
          {configs.get('toggleShortDescriptionProduct') && product && product.get('short_description') ? (
          ...
          {product && product.get('attributes') && product.get('attributes').size ? (
            <ListItem
