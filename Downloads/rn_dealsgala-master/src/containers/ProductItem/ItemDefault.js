...
export function ItemDefault(props) {
  ...
  return (
    <>
      ...
      onPress={() => {
        if(props.navigation.state.routeName == 'ProductScreen'){
          setloader(true)
        }
        navigation['navigate'](mainStack.product, {product: item})
      }}>
      <View>

        <FastImage
          source={{uri:images && images.length ? images[0].shop_single
