...
const al = ['c', 'k', 's', '2', '3', 'r'];

const jwtRequiredList = ['rnlab-app-control', 'dokan'];

const a = `${al[0]}${al[1]}_${configApi.k}${al[3]}`;
const b = `${al[0]}${al[2]}_${configApi.s}${al[4]}`;

request.interceptors.request.use(
  config => {
    config.baseURL = configApi.c + '/wp-json';
    ...
    if (checkJwt >= 0 && globalConfig.getToken()) {
      ...
      config.params = {
        [`${al[0]}onsume${al[5]}_${al[1]}ey`]: a,
        [`${al[0]}onsume${al[5]}_${al[2]}ecret`]: b,
      };
    }
    return config;
  },
